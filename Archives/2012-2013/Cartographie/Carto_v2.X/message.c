...
void message_processing_carto(unsigned char inMessage)//modifie toToDo et le copie dans toDo quand c'est complet
{
		...
		switch (tr.recep.r)
		{
			...
			case 0b0://robot Adv
			{
				...
				switch (tr.recep.d)
				{
					...
					case 0b0: //robot adv 2
					{
						toToDo.xAdv = 0;
						toToDo.yAdv = 0;
						toToDo.xAdv2 = tr.recep.x;
						toToDo.yAdv2 = tr.recep.y;
						break;
					}
				}
				break;    
	...
	#ifdef DEUX_ROBOT_ADV
	if (toToDo.xNousDep != -1 && toToDo.xNousArr != -1 && toToDo.xAdv != -1 && toToDo.xAdv2 != -1)
	#else
