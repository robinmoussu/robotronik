...
static inline void PWM1_init () {
	//P1TCONbits.PTSIDL = 1;	// Arret en pause

    P1TPER = PWM_PERIOD;		// Période du timer
    ...
    P1TCONbits.PTOPS = ASSER_PERIOD - 1;	// Reset automatique aux modif de P1TCON
}

static inline void QEI_init ()
{
	...
	DFLT1CONbits.QEOUT = 1;				// Filtre actif
	DFLT1CONbits.QECK  = 0b111;			// Horloge /256 pour le filtre
...
inline void init() {
	hardInit();
	posX.all = 0x7FFFFFFF;	// Milieu de terrain -> changé par PSoC ou callage
	posY.all = 0xFFFFFFFF/3;
