...
void drawListePoint(Point l)
{
    while(l != NULL)
    {
        if (l->f == ROND)
        {
            drawDisque((int)l->x, (int)l->y,(int)l->rayon,150,150,150);
        }
        else
        {
            drawRect((int)l->x, (int)l->y,l->rayon*2*INV_SQRT2,l->rayon*2*INV_SQRT2,150,150,150);
        }

        drawPoint((int)l->x, (int)l->y);
        l = l->queue;
    }
}

void drawPointPassage(int x, int y, Uint32 coul)
{
    x = mmToPix(x);
    y = mmToPix(y);
    SDL_Rect position;
        position.x = x - TAILLE_POINT/2 - (TAILLE_POINT%2);
        position.y = HEIGHT -(y - TAILLE_POINT/2  - (TAILLE_POINT%2));
        position.w = TAILLE_POINT;
        position.h = TAILLE_POINT;

    //SDL_BlitSurface(affich->point, NULL, affich->screen, &position);
    SDL_FillRect(affich->screen, &position, coul);
}

void drawListePointPassage(Point list)
{
    while(list != NULL)
    {   //drawDisque((int)l->x, (int)l->y,(int)l->rayon,150,150,150);
        //drawDisque((int)list->x, (int)list->y,(int)list->rayon,255,230,255);
        drawPointPassage((int)list->x, (int)list->y, getPixel(0,255,0));
        list = list->queue;
    }
}
...
void drawListeSegment(ListeSegment l,int R, int V,int B)
{
    while(l != NULL)
    {
        drawLine((int)l->a->x, (int)l->a->y, (int)l->b->x, (int)l->b->y,R,V,B);
        l = l->queue;
    }
}
