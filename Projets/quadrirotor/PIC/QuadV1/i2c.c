...
unsigned int InitI2C(void)
{
    ...
    //I2C1BRG = 0x004f;
    I2C1BRG = 0x188; //data_sheet, Fcy = Fosc/2 = 40 MHz ??
 ...
 * Note:			None
 *
 * TODO : timeout : au bout de tant de cycles on arrete d'attendre => erreur
 * + passage de data par parametre pour retourner l'erreur
 * -> a moitié fait ..., laisser comme ça pour l'instant, ça marche !
 ********************************************************************/
unsigned int getI2C(void)
{
    ...
    {
        cpt++;
        if (cpt > I2C_DEFAULT_READ_TIMEOUT)
        {
            //cpt++;
            //debugLed1(ON); TODO
            break;
